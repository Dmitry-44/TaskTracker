...
export const GetAllOperations = (payload?: FilterPayload) => {
    return axiosClient.post(`${envConfig.API_URL}tasktracker/operations`, payload)
}

export const SendOperation = (payload: Partial<Operation>|Operation) => {
    const api = payload?.id ? `${envConfig.API_URL}tasktracker/operation/${payload?.id}` : `${envConfig.API_URL}tasktracker/pipe`
